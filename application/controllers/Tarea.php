...
class Tarea extends CI_Controller
{
	...
	public function index($permission = null)
	{
		...
		if (empty($data['user_data'][0]['usrName'])) {
			...
			// Limpiamos cache para forzar recarga desde ajax de Datatables
			$_SESSION['listadoTareas_cache'] = null;
	...
	public function verificarInforme()
	{
		...
		if ($result['status']) {
			...
			if ($opcion == "true") {
				...
				if ($nextTask == 0) {
					echo json_encode(msj(false, 'No se pudo Obtener Tarea Siguiente | Confecciona informe servicio'));
					return;
				}
	...
	//actualiza $_SESSION['listadoTareas_cache'] reseteandolo
	function actualizaTareas()
	{
		$data = $this->session->userdata();
        
        // Limpiamos cache para forzar recarga en el datatable 
        $_SESSION['listadoTareas_cache'] = null;
        $_SESSION['bonita_page'] = 0;
        $_SESSION['bonita_exhausted'] = false;
        $_SESSION['bonita_search'] = '';
	}
