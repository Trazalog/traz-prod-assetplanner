...
class Ordenservicio extends CI_Controller
{
  ...
  {
    $selected_orders = $this->input->post('selected');
    $results = [];

    if (empty($selected_orders)) {
      echo json_encode(['status' => false, 'msj' => 'No se seleccionaron órdenes']);
      return;
    }

    // Obtener tareas pendientes del usuario desde BPM
    $rsp = $this->bpm->getTodoList();
    $bpm_tasks = $rsp['status'] ? $rsp['data'] : [];
    ...
    foreach ($selected_orders as $order) {
      ...
      $id_ot = $order['id_ot'];
      $id_orden_servicio = $order['id_orden'];

      // Buscar la tarea en BPM para este case_id
      $idTarBonita = 0;
      foreach ($bpm_tasks as $task) {
        if ($task['caseId'] == $case_id && $task['displayName'] == "Verifica Informe de Servicio") {
          $idTarBonita = $task['id'];
          break;
        }
      }
      ...
      if ($idTarBonita != 0) {
        // Tomar y cerrar la tarea
        $this->bpm->setUsuario($idTarBonita, $usuarioBPM);
        $res_bpm = $this->bpm->cerrarTarea($idTarBonita, $opcionSel);

        if ($res_bpm['status']) {
          // Actualizar lectura historial
          $lecturas = $this->Ordenservicios->getLecturasOrden($id_ot);
          if ($lecturas) {
            $data_lectura = array(
              "id_equipo" => $order['id_equipo'],
              "lectura" => $lecturas[0]['horometrofin'],
              "fecha" => date('Y-m-d H:i:s'),
              "usrId" => $usrId,
              "observacion" => 'Verificación Masiva Informe Servicio | OT: ' . $order['id_ot'],
              "operario_nom" => "-",
              "turno" => "-",
              "estado" => "AC"
            );
            $this->Tareas->setUltimaLecturaIS($data_lectura);
          }

          // Cambiar estados
          $this->Tareas->cambiarEstado($id_ot, 'CE', 'informe servicios');
          $this->Tareas->cambiarEstado($id_ot, 'CE', 'OT', true);

          $results[] = ['id_orden' => $id_orden_servicio, 'status' => 'OK'];
        } else {
          $results[] = ['id_orden' => $id_orden_servicio, 'status' => 'ERROR_BPM', 'msj' => $res_bpm['msj']];
        }
      } else {
        // Si no hay tarea en BPM, igual intentamos actualizar estados
        $this->Tareas->cambiarEstado($id_ot, 'CE', 'informe servicios');
        $this->Tareas->cambiarEstado($id_ot, 'CE', 'OT', true);
        $results[] = ['id_orden' => $id_orden_servicio, 'status' => 'OK_NO_BPM'];
      }
    }
