                  ...
                  foreach($list as $a){
                    if ($a['estado'] =='P'){
                      ...
                      if (strpos($permission,'Add') !== false) { //Entregar
                        echo '<i class="fa fa-check-circle-o text-light-blue" style="cursor: pointer; margin-left: 15px;"  title="Entregar Pedido " data-toggle="modal" data-target="#modalpedido"></i>';
                      }
                      echo '</td>';
                      echo '<td>'.$a['nro_trabajo'].'</td>';
                      echo '<td>'.$a['fecha'].'</td>';
                      echo '<td>'.$a['fecha_entrega'].'</td>';
                      echo '<td>'.$a['provnombre'].'</td>';
                      echo '<td>'.$a['descripcion'].'</td>';
                      echo '<td>'.($a['estado'] == 'C' ? '<small class="label pull-left bg-green">Curso</small>' : ($a['estado'] == 'P' ? '<small class="label pull-left bg-red">Pedido</small>' : ($a['estado'] == 'As' ? '<small class="label pull-left bg-yellow">Asignado</small>' : '<small class="label pull-left bg-blue">Terminado</small>'))).'</td>';
                      echo '</tr>';
                    }
                  }
                }
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <h4 class="modal-title"  id="myModalLabel"><span id="modalAction" class="fa fa-check-circle-o text-light-blue"></span> Número de remito </h4>
       </div> <!-- /.modal-header  -->
