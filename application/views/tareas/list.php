                ...
                <div class="box-header">
                    ...
                    <?php if (isset($empr_id) && $empr_id == '6'): ?>
                        ...
                        <div style="margin-bottom:10px; display:inline-block;">
                            ...
                            <input type="datetime-local" id="fechaCorte" name="fechaCorte" value="">
                        </div>
                        <button id="btn-cierre-masivo-confecciona" class="btn btn-primary">Cierre masivo Confecciona
                            informe</button>
...
<script>
    ...
    $('#btn-cierre-masivo-confecciona').on('click', function () {
        var fechaCorte = $('#fechaCorte').val();
        if (!fechaCorte) {
            alert('Por favor, ingrese la fecha de corte.');
            return;
        }
        WaitingOpen('Cerrando tareas masivamente...');
        $.ajax({
            url: 'Tarea/cierreMasivoConfeccionaInforme',
            type: 'POST',
            data: { fecha_corte: fechaCorte },
            success: function (data) {
                WaitingClose();
                alert('Cierre masivo Confecciona informe realizado');
                console.log(data);
            }
        });
    });

    $('#btn-cierre-masivo-verifica').on('click', function () {
        var fechaCorte = $('#fechaCorte').val();
        if (!fechaCorte) {
            alert('Por favor, ingrese la fecha de corte.');
            return;
        ...
        WaitingOpen('Cerrando tareas masivamente...');
        $.ajax({
            url: 'Tarea/cierreMasivoVerificaInforme',
            type: 'POST',
            data: { fecha_corte: fechaCorte },
            success: function (data) {
                WaitingClose();
