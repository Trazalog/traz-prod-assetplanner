              ...
              <?php
               ...
               if (strpos($permission,'Edit') !== false) {
                  ...
                  if (strpos($permission,'Del') !== false) {
                   echo '<i class="fa fa-fw fa-times-circle text-light-blue" style="cursor: pointer; margin-left: 15px;" title="Eliminar" onclick="Loadprovee('.$f['provid'].',\'Del\')"></i>';
                  }
                  echo '</td>';
                  echo '<td>'.$f['provid'].'</td>';
                  ...
                  echo '<td style="text-align: center">'.($f['estado'] == 'AC' ? '<small class="label pull-left bg-green">Activo</small>' : ($f['estado'] == 'AN' ? '<small class="label pull-left bg-red">Inactivo</small>' : '<small class="label pull-left bg-yellow">Suspendido</small>')).'</td>';
...
<script>
  ...
  function guardarProveedor(){
    ...
    function editarProveedor(){
      ...
      var hayError = false;
        if($('#provnombreE').val() == '' || $('#provcuitE').val() == '' || $('#provdomicilioE').val() == '' || $('#provtelefonoE').val() == '' || $('#provmailE').val() == '' || $('#estadoE').val() == '' || $('#id_empresaE').val() == '' )
        {
          hayError = true;
        }
        if(hayError == true){
    ...
    WaitingOpen();
     $.ajax({
       type: 'POST',
       dataType : "json",
       data: {"provid" : id,  "provnombre":provnombre,  "provcuit":provcuit,  "provdomicilio":provdomicilio,  "provtelefono":provtelefono,  "provmail":provmail,  "estado":estado,  "id_empresa":id_empresa },
